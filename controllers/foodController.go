	...
	return func(c *gin.Context) {
		...
		projectStage := bson.D{
			{
				Key: "$project", Value: bson.D{
					...
					{Key: "food_item", Value: bson.D{{Key: "$slice", Value: []interface{}{startIndex, recordsPerPage}}}},
				},
			},
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "error occured while listing items",
			})
			return
		}
		var allFoods []bson.M
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			msg := fmt.Sprintf("menu has not found")
		...
		if insertError != nil {
			msg := fmt.Sprintf("Food item has not created")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
		defer cancel()
